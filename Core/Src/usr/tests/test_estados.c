...
int testes_estados(void)
{
  ...
  modo = estados_atualiza(&es, &cfg, canais, 1, 0, t);
  ...
  t += 1000;
  modo = estados_atualiza(&es, &cfg, canais, 0, 0, t);
  if (modo != ESTADO_ESPERA)
  {
  ...
  t += 1;
  (void)estados_atualiza(&es, &cfg, canais, 1, 0, t);
  t += cfg.tempo_gesto_ms + 1;
  modo = estados_atualiza(&es, &cfg, canais, 1, 0, t);
  if (modo != ESTADO_VOO)
  {
  ...
  t += cfg.tempo_switch_ms + 1;
  modo = estados_atualiza(&es, &cfg, canais, 1, 1, t);
  if (modo != ESTADO_ESPERA)
  {
